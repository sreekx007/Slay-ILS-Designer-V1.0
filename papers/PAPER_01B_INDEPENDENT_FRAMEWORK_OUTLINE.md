...
| GD-ST | Top-frame subassembly that anchors the terminal feature of every GD-B branch. |
| GD-SB | Protection/support assembly for valve roller/contact load assumptions. |
...
Planned case-example requirements:

- Examples must be preselected rather than taken from exploratory conversations.
- Each example must include confirmed EDPR understanding, retrieved context, deterministic gate output, plot/report artifacts where applicable, and independent review.
- The paper should report successful emissions and correct typed gaps only after the workflow is stable.

### 9.1 Prototype Boundary

The repository is still a developing implementation. Its current role in the paper is to define architecture, schemas, gates, and artifact requirements. Repository tests and local trials should support software development, but they should not be presented as research results until the controlled example package is executed.
