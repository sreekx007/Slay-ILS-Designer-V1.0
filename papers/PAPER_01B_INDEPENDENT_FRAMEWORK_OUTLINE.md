...
- Every GD-B branch must terminate at GD-ST through an explicit association; L terminals are horizontal, Z terminals are vertical, and the selected anchor defines the compatible frame feature.
- Connector orientation is a separate constraint: a vertical connector requires a Z branch rather than an arbitrary routing.
...
- GD-ST parameters and branch associations must be explicit and plotted.
...
L/Z branch anchoring to GD-ST, vertical-connector Z selection, GD-ST report completeness, GD-SB valve-protection
