...
This architecture also makes failure diagnosis possible. If the output is weak, the root cause can be located: problem parsing, component definition, assembly representation, evidence retrieval, evidence applicability, deterministic tool execution, visualization, conversation workflow, or missing data. This diagnostic property became central during KEL development, because many observed errors were not primarily model-intelligence failures. They were failures to enforce a gate that the system already knew should exist.

The latest repository work adds a further practical distinction. Official EDIKB behavior nodes and datasets represent reviewed knowledge, while `knowledge/edikb/future_study_candidates/` records unresolved FEA, ML, design-of-experiments, and correlation-study candidates. These candidates are not treated as official behavior rules. They are a governed backlog of questions that must be answered before the framework can make stronger optimized-design claims.
...
| Model-anchored annotation gate | Require strain-watch arrows and other callouts to anchor to model coordinates or component features rather than pixel judgement. |
| Structure connection label gate | Require F/P/S/D non-weld structural connection-type labels for GD-ST/GD-SB review, while avoiding unnecessary connector component labels. |
| Feedback-loop gate | Require the assistant to ask for human feedback after a plotted layout so KEL can capture corrections. |
...
The recent plot-annotation work illustrates why tool availability alone is not enough. The repository already had a plotter, but a workflow could still place peak-strain or transition pointers by visual judgement after rendering. That creates an apparently precise figure without a model-coordinate basis. The new contract resolves annotations through live component features or explicit model coordinates and reports unresolved anchors as failures. Similarly, connection-label feedback clarified that the required review information is the structural connection type, not a label on every connector component. This type of correction is small in code but large in engineering significance: it prevents the visual artifact from claiming precision it does not have.

## 8. Knowledge Evolution Loop

KEL is the active governance mechanism for learning from experience. It extends the front-end idea of capturing feedback into a full lifecycle for knowledge evolution. In the latest repository work, KEL is also the method used to test whether the workflow itself has loopholes. It does not only store comments; it asks what went wrong, what available knowledge was missed, whether the current EDIKB is insufficient, and what gate, record, or future study is needed.
...
| Q8 | Plot annotations must be anchored to model geometry; connection-label scope must be explicit; post-plot feedback must be requested. |
| Q9 | GD-ST frame length and P-S connector spacing must be classified as precedent/containment-based unless correlation evidence supports an optimized low-strain claim. |
...
The root-cause log separates four situations that would otherwise be mixed together:

| Root-cause type | Example | Framework response |
|---|---|---|
| Workflow bypass | The LLM answers before EDPR confirmation or delivers a plot without asking for feedback. | Add conversation and governance gates. |
| Available knowledge missed | GD-VLV plus GD-SH is not connected to analogous GD-TP/GD-TT plus GD-SH evidence. | Enforce retrieval-usefulness checks and evidence-application gates. |
| Tool-output loophole | Annotation arrows are placed by visual judgement rather than model coordinates. | Add plot contracts, metadata, and unresolved-anchor failures. |
| Genuine evidence gap | GD-ST length and P-S connector spacing lack correlation data for low-strain branch-valve sizing. | Create future FEA/ML/correlation study candidates instead of claiming optimization. |

This loop is one of the most important practical findings of the project. An EDIKB and a toolchain can still leave loopholes: the LLM may skip a tool, misuse an output, fail to apply retrieved evidence, or treat a precedent as an optimized result. KEL provides a disciplined method to discover those holes, classify them, plug them with gates when possible, and record future study candidates when evidence is genuinely missing.
...
| GD-ST connector spacing under low-strain claim | Demonstrate the distinction between EDAS precedent, containment-based sizing, and correlation-optimized sizing. | EDIKB evidence check, Q9 feedback record, future-study candidate, governance statement that optimization is not yet proven. |
...
The final paper can then measure artifact completeness, gate outcomes, traceability, evidence citation, plot authority, human remaining actions, and whether missing correlation evidence is explicitly converted into future-study candidates. This is more defensible than claiming that a single chat example proves design quality.
...
The latest repository work makes this explicit by creating a browsable future-study candidate backlog under EDIKB. These records are not official behavior rules. They are structured candidates for future FEA, design-of-experiments, correlation analysis, or ML work. They should be promoted only after expert review and verified dataset rows.

Current candidate types include:

| Future study candidate | Why it matters |
|---|---|
| Branch valve mass, connector system, and header strain redistribution covariance | Needed before claiming that a branch-valve connector system is globally low-strain optimal. |
| GD-SB/GD-ST support dimension versus pipeline strain covariance | Needed before claiming support dimensions are strain-optimal rather than merely fitted to component envelopes. |
| GD-ST length and P-S connector spacing correlation for ILT-L branch-valve layouts | Needed before claiming low-strain optimized GD-ST length or connector spacing for a branch-valve layout. |
| GD-VLV plus GD-SB contact, connector selection, and valve moment evidence | Needed before accepting final valve/base connector selection and valve moment utilization. |
| Direct GD-VLV plus GD-SH stiff-component interaction evidence | Needed before claiming valve-specific shroud placement is optimized rather than analogical. |
...
This is a stronger position than merely saying "more data is needed." The framework records what data is needed, why it is needed, which components and variables are involved, which responses should be measured, and what claim cannot be made until the evidence exists.
...
This also changes how "design intuition development" should be described. The system should not claim intuition because the LLM produced a plausible recommendation. Instead, the framework accumulates structured experience: what was asked, what evidence was retrieved, what available correlation was missed, what gate failed, what root cause was found, and what future study is required. Over time, this creates a supervised pathway from human review and tool failures to more reliable rules, richer datasets, and eventually ML-assisted prediction.

## 12. Hybrid RAG and Vector Retrieval as a Future Improvement

The current framework is intentionally structured around symbolic engineering records: EDPR, EDES, EDAS, EDIKB, KEL records, schemas, gates, and deterministic tools. This structure is necessary for engineering authority. However, symbolic records introduce a practical natural-language loophole. Human users rarely phrase design problems using exact repository identifiers. Different organizations may also use different names for the same or related concepts. One company may say "strongback," another may say "top frame," while the repository term is `GD-ST`. One engineer may say "the valve cannot ride rollers," while the framework must map that statement to `GD-VLV`, valve roller-contact limitations, GD-SB protection, missing roller geometry, valve moment evidence, and KEL lessons about valve-protection gates.

This means that exact keyword or ID matching is not enough. A future hybrid RAG implementation can help by using vector retrieval as the semantic bridge between varied human language and the controlled ontology.

Vector retrieval should be used for semantic recall, not final authority. It can retrieve candidate records that are semantically close to a query, even when the words do not match exactly. Symbolic grounding should then resolve those candidates to official IDs and structured JSON records. Deterministic gates decide whether the candidates are valid, applicable, complete, and sufficient.

The intended flow is:

```mermaid
flowchart TD
    A["Human query"] --> B["Vector retrieval"]
    B --> C["Candidate EDPR/EDES/EDAS/EDIKB/KEL records"]
    C --> D["Symbolic grounding to IDs"]
    D --> E["Schema and design gates"]
    E --> F["Governed output or gap"]
```
...
|---|---|
| "strongback" or "top frame" | `GD-ST`, EDAS top-structure associations, GD-ST connector slots. |
| "base frame", "stiffener bracket", or "roller protection" | `GD-SB`, valve/base protection gates, roller-contact assumptions. |
| "valve cannot ride rollers" | `GD-VLV`, GD-SB requirement, missing clearance/load/moment inputs. |
| "vertical connector" | `GD-B.variant = Z`, `ILT-Z-*` anchors, branch-to-GD-ST association. |
| "branch connector support" | `GD-B`, `GD-ST`, `GD-Con`, terminal feature association. |
| "similar past mistake" | KEL root-cause records and implemented workflow gates. |
| "is this sizing optimized?" | EDIKB evidence rows, future-study candidates, and correlation-gap records. |

This is particularly important in engineering organizations because source knowledge is not only in structured datasets. It is distributed across reports, emails, review comments, design notes, calculation narratives, meeting minutes, drawings, spreadsheets, and local terminology. Vector indexing can help build the knowledge base by surfacing semantically related fragments from this natural-language material. Those fragments can then be converted into candidate EDES, EDAS, EDIKB, EDPR, or KEL records for review.

The proposed hybrid RAG improvement has three layers:

| Layer | Role |
|---|---|
| Vector retrieval | Finds semantically related text chunks, records, prior problems, feedback, and evidence candidates. |
...
| Governance and gates | Determine whether the grounded records may be used, whether evidence is sufficient, or whether a KEL/future-study candidate is required. |

This section is important because vectorization can reduce one class of workflow error: misunderstanding or under-retrieving relevant knowledge because the user used different language from the repository. It can also help discover analogies, such as retrieving shroud plus thick-component evidence for a valve plus shroud request. However, vector retrieval must remain subordinate to symbolic validation. A semantically similar paragraph is not an engineering rule. It becomes useful only after grounding, evidence checking, and expert-governed promotion.

Therefore, the future target is not pure vector RAG. It is:

> hybrid RAG for semantic recall, neuro-symbolic grounding for engineering meaning, deterministic gates for validity, and KEL for continuous loophole detection and knowledge evolution.

## 13. Professional Engineering Practice Relevance
...
- Hybrid vector retrieval is a future enhancement; current structured retrieval does not yet solve every natural-language terminology variation.
...
- implementing hybrid RAG with vector indexes over EDPR, EDES, EDAS, EDIKB, KEL, reports, review comments, and future-study candidates;
- adding symbolic grounding from retrieved text chunks to official framework IDs and records;
