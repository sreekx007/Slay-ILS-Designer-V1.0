...
- For a branch valve, populate `workflowIntent.branchValve`. The solver must select a compatible `ILT-L-*` or `ILT-Z-*` standard anchor, keep the branch tee/entry, branch valve, and branch end inside the associated `GD-ST` span, and avoid `F2`/`F2D` unless explicit EDIKB/numeric evidence justifies the low-strain branch pocket and accepts the header-strain penalty. Prefer `PS` where no low-strain-pocket evidence is required.
